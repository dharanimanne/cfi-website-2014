...
abstract class DBDataWrapper extends DataWrapper{
	...
	*/
	...
	public function select($source){
		...
		return $this->query($this->select_query($select,$source->get_source(),$where,$sort,$source->get_start(),$source->get_count(),$source->get_activity_id()));
	}	
	...
	*/
	protected function select_query($select,$from,$where,$sort,$start,$counts,$activity_id){ 
		if (!$from)
			return $select;
		...
		$sql .= " WHERE `activity_id` = $activity_id";  echo $sql; 
		return $sql;
	}
	...
	*/
	protected function update_query($data,$request){
		...
		$sql.=" AND `activity_id`=".$request->get_activity_id();
	...
	*/
	protected function insert_query($data,$request){
		...
		$sql="INSERT INTO ".$request->get_source()."(".implode(",",$temp_n).", activity_id) VALUES (".implode(",",$temp_v).", $activity_id)";
